  ...
  require 'vendor/PHPMailer/src/SMTP.php';
	...
	if(isset($_POST['login'])) {
		...
		if(mysqli_num_rows($check)===1) {
				...
				$check = mysqli_query($conn, "SELECT * FROM users WHERE email='$email' AND password='$userpassword'");
				if(mysqli_num_rows($check)===1) {
...
// REGISTER USER
	if(isset($_POST['created_user'])) {
		...
		$Name_Siblings          = mysqli_real_escape_string($conn, $_POST['Name_Siblings']);
		$School_Place_Work      = mysqli_real_escape_string($conn, $_POST['School_Place_Work']);
		$Siblings_Age           = mysqli_real_escape_string($conn, $_POST['Siblings_Age']);
    ...
    {  
       $all_Parent_status .= $pStatus.",";  
		...
		$School_Org             = mysqli_real_escape_string($conn, $_POST['School_Org']);
		$School_Position        = mysqli_real_escape_string($conn, $_POST['School_Position']);
		


		$OutsideSchool_Org      = mysqli_real_escape_string($conn, $_POST['OutsideSchool_Org']);
		$OutsideSchool_Position = mysqli_real_escape_string($conn, $_POST['OutsideSchool_Position']);
              ...
              if($save) {
              	foreach($Name_Siblings as $key1 => $value) {
								...
								foreach($School_Org as $key2 => $value2) {
											$sch_Org = mysqli_query($conn, "INSERT INTO siblingsrecord (user_Id, School_Org, School_Position) VALUES ('".$user_Id."' ,'".$value2."', '".$School_Position[$key2]."') ");
										}
								foreach($OutsideSchool_Org as $key3 => $value3) {
											$out_sch_Org = mysqli_query($conn, "INSERT INTO siblingsrecord (user_Id, OutsideSchool_Org, OutsideSchool_Position) VALUES ('".$user_Id."' ,'".$value3."', '".$OutsideSchool_Position[$key3]."') ");
										}

              	$subject = 'Verification code';
			          $message = '<p>Good day sir/maam '.$email.', your password is <b>'.$key.'</b>. Please do not share this password to other people. Thank you!</p>
			          ...
			          try {
			            ...
			            $mail->SMTPAuth = true;                             
		              $mail->Username = 'isujentranceexam@gmail.com';     
            			$mail->Password = 'cgvmnkftvysogqqk';            
			            ...
			            //Send Email
			            $mail->setFrom('isujentranceexam@gmail.com');

			            //Recipients
			            $mail->addAddress($email);              
			            $mail->addReplyTo('isujentranceexam@gmail.com');
                ...
                $_SESSION['message'] = "Something went wrong while saving the information. Please try again.";
                  ...
                  // if everything is ok, try to upload file
                      ...
                      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                      	...
                      	$save = mysqli_query($conn, "INSERT INTO users (school_year, semester, enrollmentStatus, firstname, middlename, lastname, suffix, gender,dob, age, place_of_birth, address, bh_Address, landline, email, contact, birth_order, civilstatus, school, language, religion, nationality, spouseName, no_of_Children, F_Name, F_Address, F_Landline, F_Contact, F_Education, F_Occupation, F_Annual_Income, F_Religion, M_Name, M_Address, M_Landline, M_Contact, M_Education, M_Occupation, M_Annual_Income, M_Religion, Parent_status, G_Name, G_Address, G_Landline, G_Contact, G_Relation, Emerge_Name, Emerge_Address, Emerge_Contact, Elem_School, Elem_Place, Elem_DateAttended, HS_School, HS_Place, HS_DateAttended, College_School, College_Place, College_DateAttended, Voc_School, Voc_Place, Voc_DateAttended, easySubject, difficultSubject, Sub_lowestGrade, Sub_HighestGrade, Awards, GWA, NatureSchooling, InterruptedforHowLongReason, Interests, Skills, Hobbies, Characteristics, presentProblems, presentFears, ChronicIllness, Med_Reg_taken, Accidents_exp, Operation_exp, image) VALUES ('$school_year', '$semester', '$enrollmentStatus', '$firstname', '$middlename', '$lastname', '$suffix', '$gender','$dob', '$age', '$place_of_birth', '$address', '$bh_Address', '$landline', '$email', '$contact', '$birth_order', '$civilstatus', '$school', '$language', '$religion', '$nationality', '$spouseName', '$no_of_Children', '$F_Name', '$F_Address', '$F_Landline', '$F_Contact', '$F_Education', '$F_Occupation', '$F_Annual_Income', '$F_Religion', '$M_Name', '$M_Address', '$M_Landline', '$M_Contact', '$M_Education', '$M_Occupation', '$M_Annual_Income', '$M_Religion', '$Parent_status', '$G_Name', '$G_Address', '$G_Landline', '$G_Contact', '$G_Relation', '$Emerge_Name', '$Emerge_Address', '$Emerge_Contact', '$Elem_School', '$Elem_Place', '$Elem_DateAttended', '$HS_School', '$HS_Place', '$HS_DateAttended', '$College_School', '$College_Place', '$College_DateAttended', '$Voc_School', '$Voc_Place', '$Voc_DateAttended', '$easySubject', '$difficultSubject', '$Sub_lowestGrade', '$Sub_HighestGrade', '$Awards', '$GWA', '$NatureSchooling', '$InterruptedforHowLongReason', '$Interests', '$Skills', '$Hobbies', '$Characteristics', '$presentProblems', '$presentFears', '$ChronicIllness', '$Med_Reg_taken', '$Accidents_exp', '$Operation_exp', '$file')");
